...
LaplacesDemon_dmvt <- function(x, mu, S, df=Inf, log=FALSE)
{
...
TruncatedNormal_pmvnorm <- function(mu, sigma, lb = -Inf, ub = Inf, B = 1e4, type = c("mc", "qmc"), check = TRUE, ...){
...
logliktArplmec <- function(nu,y,x,z,cc,ttc,nj,LL,LU,betas,sigmae,D1,pii){
  ...
  for(j in 1:m)
  {
    ...
    if(sum(cc1)==0)
    {
      ...
      ver[j,] <- suppressWarnings(LaplacesDemon_dmvt(x = as.vector(y1), mu = as.vector(muii), S = as.matrix(SIGMA), df = nu ))
    ...
    if(sum(cc1)>=1)
    {
      ...
      if(sum(cc1)==nj[j])
      {
        ...
        ver[j,] <- suppressWarnings(LaplacesDemon_dmvt(x = as.vector(y1[cc1==0]),mu =as.vector(muii[cc1==0]),S =as.matrix(SIGMA[cc1==0,cc1==0]),df= nu)*as.numeric(TruncatedNormal::pmvt(lb = as.vector(LL1c),ub=as.vector(LU1c), mu = as.vector(muiic),df=nu, sigma = as.matrix(Sc0))))
  ...
  for(j in 1:m)
  {
    ...
    if(sum(cc1)>=1)
    {
      ...
      if(sum(cc1)==nj[j])
      {
       ...
       ver[j,] <- suppressWarnings(TruncatedNormal_pmvnorm(lb = as.vector(LL1),ub=as.vector(LU1),
        ...
        ver[j,] <- suppressWarnings(LaplacesDemon::dmvn(x = as.vector(y1[cc1==0]),mu =as.vector(muii[cc1==0]),
                                      ...
                                      as.numeric(TruncatedNormal_pmvnorm(lb= as.vector(LL1c),ub=as.vector(LU1c), mu = as.vector(muiic),
                                                                  sigma = as.matrix(Sc0))))
  ...
  for(j in 1:m)
  {
    ...
    if(sum(cc1)>=1)
    {
      ...
      if(sum(cc1)==nj[j])
      {
        ...
        ver[j,] <- suppressWarnings(TruncatedNormal_pmvnorm(lb = as.vector(LL1),ub=as.vector(LU1), mu = as.vector(muii),sigma = as.matrix(SIGMA)))
        ...
        ver[j,] <- suppressWarnings(LaplacesDemon::dmvn(x = as.vector(y1[cc1==0]),mu =as.vector(muii[cc1==0]),Sigma =as.matrix(SIGMA[cc1==0,cc1==0]))*
                                      as.numeric(TruncatedNormal_pmvnorm(lb = as.vector(LL1c),ub=as.vector(LU1c), mu = as.vector(muiic),sigma = as.matrix(Sc0))))
