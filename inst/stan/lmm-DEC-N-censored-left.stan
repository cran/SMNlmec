functions{
	...
	matrix DEC_matrix(real phi1_set, real phi2_set, int nj, vector tt) { 
     matrix[nj, nj] mat;
     real lag;
     ...
     for(i in 1:nj){
       for(j in 1:nj){
         lag = fabs(tt[i]-tt[j]);
         mat[i,j] = pow(phi1_set, pow(lag,phi2_set));
       }
     }     
...
parameters {
      ...
      real<lower=0> sigmae;
		  real<lower=0, upper= 0.9999> phi1;
		  real<lower=0> phi2;
...
model {
  ...
  real sigma_sqrt;
	...
	for (j in 1:n){
		{
		  matrix[njvec[j],njvec[j]] Sigma_Ri = DEC_matrix(phi1,phi2,njvec[j],timevar[(njloc+1):(njloc + njvec[j])]);
		  matrix[njvec[j],njvec[j]] Sigma = sigma2 * Sigma_Ri;
